...
public class DaishoSayaObiTrinketRenderer implements TrinketRenderer {
	...
	// (bones named "armorLeftLeg" auto-snap to the leg ModelPart) rather than
	// hand-tuned PoseStack offsets.
	private <R extends HumanoidRenderState & GeoRenderState> void renderArmor(HumanoidModel<?> model,
		...
		GeoArmorRenderer rawRenderer = GeoRenderProvider.of(stack).getGeoArmorRenderer(stack, EquipmentSlot.LEGS);
		if (rawRenderer == null) {
			return;
		}

		GeoArmorRenderer.RenderData renderData = new GeoArmorRenderer.RenderData(stack, EquipmentSlot.LEGS, entity, model);
		@SuppressWarnings("unchecked")
		GeoRenderState filledState = rawRenderer.fillRenderState(animatable, renderData, renderState, 1.0F);
		rawRenderer.performRenderPass(filledState, poseStack, collector, null);
	}
}
